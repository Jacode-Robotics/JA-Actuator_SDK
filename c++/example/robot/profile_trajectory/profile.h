...
void PF_Handle::NewGoalPos(int32_t starting_position, int32_t goal_position) {
    ...
    case 0:
        t1 = 600 * profile_vel / profile_acc * 10;
        t2 = (6000000 / 32768) * (static_cast<float>(delta_pos) / static_cast<float>(profile_vel)) * 10;
        if (t1 > t2) {
            t1 = sqrt(static_cast<float>(delta_pos) / static_cast<float>(profile_acc) / 32768) * 600000;
            t2 = t1;
        }
        t3 = t1 + t2;
...
bool PF_Handle::ExecutionPos() {
    ...
    if (t >= t3) {
        ...
        f_trajectory_vel += f_trajectory_acc * 0.0001666667;
        f_trajectory_pos += f_trajectory_vel * 0.000546133;
