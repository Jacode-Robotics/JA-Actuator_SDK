...
def move1():
    ...
    data = [0xFF,0xFF,0xFD,0x00,0xFE,0x2A,0x00,0x83,0x34,0x02,0x04,0x00,0x01,0x00,0x00,0x00,0x00,0x02,0x00,0x00,0x00,0x00,0x03,0x00,0x00,0x00,0x00,0x04,0x00,0x00,0x00,0x00,0x05,0x00,0x00,0x00,0x00,0x06,0x00,0x00,0x00,0x00,0x07,0x00,0x00,0x00,0x00,0x00,0x00]
    crc = update_crc(data)
    ...
    if written_bytes != len(byte_data):
        ...
        print("move1 succeeded")

def move2():
    crc = 0x0000
 
    data = [0xFF,0xFF,0xFD,0x00,0xFE,0x2A,0x00,0x83,0x34,0x02,0x04,0x00,0x01,0x00,0x00,0x00,0x00,0x02,0x00,0x00,0x00,0x00,0x03,0x00,0x20,0x00,0x00,0x04,0x00,0x00,0x00,0x00,0x05,0x00,0x20,0x00,0x00,0x06,0x00,0x00,0x00,0x00,0x07,0x00,0x00,0x00,0x00,0x00,0x00]
    crc = update_crc(data)
    data[len(data)-2] = crc & 0xff
    data[len(data)-1] = (crc >> 8) & 0xff
    ...
    if written_bytes != len(byte_data):
        ...
        print("move2 succeeded")

def move3():
    crc = 0x0000
 
    data = [0xFF,0xFF,0xFD,0x00,0xFE,0x2A,0x00,0x83,0x34,0x02,0x04,0x00,0x01,0x48,0xF4,0xFF,0xFF,0x02,0x04,0x10,0x00,0x00,0x03,0x04,0x10,0x00,0x00,0x04,0x48,0xF4,0xFF,0xFF,0x05,0x78,0x1E,0x0,0x00,0x06,0x18,0xFC,0xFF,0xFF,0x07,0xB8,0x0B,0x00,0x00,0x00,0x00]
    crc = update_crc(data)
    data[len(data)-2] = crc & 0xff
    data[len(data)-1] = (crc >> 8) & 0xff
    ...
    if written_bytes != len(byte_data):
        ...
        print("move3 succeeded")

def move4():
    crc = 0x0000
 
    data = [0xFF,0xFF,0xFD,0x00,0xFE,0x2A,0x00,0x83,0x34,0x02,0x04,0x00,0x01,0xB8,0x0B,0x00,0x00,0x02,0x04,0x10,0x00,0x00,0x03,0x70,0x17,0x00,0x00,0x04,0x88,0x13,0x00,0x00,0x05,0x78,0x1E,0x00,0x00,0x06,0x18,0xFC,0xFF,0xFF,0x07,0x00,0x00,0x00,0x00,0x00,0x00]
    crc = update_crc(data)
    data[len(data)-2] = crc & 0xff
    data[len(data)-1] = (crc >> 8) & 0xff
    ...
    if written_bytes != len(byte_data):
        ...
        print("move4 succeeded")

def move5():
    crc = 0x0000
 
    data = [0xFF,0xFF,0xFD,0x00,0xFE,0x2A,0x00,0x83,0x34,0x02,0x04,0x00,0x08,0x00,0x00,0x00,0x00,0x09,0x00,0x00,0x00,0x00,0x10,0x00,0x00,0x00,0x00,0x11,0x00,0x00,0x00,0x00,0x12,0x00,0x00,0x00,0x00,0x13,0x00,0x00,0x00,0x00,0x14,0x00,0x00,0x00,0x00,0x00,0x00]
    crc = update_crc(data)
    data[len(data)-2] = crc & 0xff
    data[len(data)-1] = (crc >> 8) & 0xff
    ...
    if written_bytes != len(byte_data):
        ...
        print("move5 succeeded")

def move6():
    crc = 0x0000
 
    data = [0xFF,0xFF,0xFD,0x00,0xFE,0x2A,0x00,0x83,0x34,0x02,0x04,0x00,0x08,0x00,0x00,0x00,0x00,0x09,0x00,0x00,0x00,0x00,0x10,0x00,0x20,0x00,0x00,0x11,0x00,0x00,0x00,0x00,0x12,0x00,0x20,0x00,0x00,0x13,0x00,0x00,0x00,0x00,0x14,0x00,0x00,0x00,0x00,0x00,0x00]
    crc = update_crc(data)
    data[len(data)-2] = crc & 0xff
    data[len(data)-1] = (crc >> 8) & 0xff
    ...
    if written_bytes != len(byte_data):
        ...
        print("move6 succeeded")

def move7():
    crc = 0x0000
 
    data = [0xFF,0xFF,0xFD,0x00,0xFE,0x2A,0x00,0x83,0x34,0x02,0x04,0x00,0x08,0x48,0xF4,0xFF,0xFF,0x09,0x04,0x10,0x00,0x00,0x10,0x04,0x10,0x00,0x00,0x11,0x48,0xF4,0xFF,0xFF,0x12,0x78,0x1E,0x0,0x00,0x13,0x18,0xFC,0xFF,0xFF,0x14,0xB8,0x0B,0x00,0x00,0x00,0x00]
    crc = update_crc(data)
    data[len(data)-2] = crc & 0xff
    data[len(data)-1] = (crc >> 8) & 0xff
    ...
    if written_bytes != len(byte_data):
        ...
        print("move7 succeeded")

def move8():
    crc = 0x0000
 
    data = [0xFF,0xFF,0xFD,0x00,0xFE,0x2A,0x00,0x83,0x34,0x02,0x04,0x00,0x08,0xB8,0x0B,0x00,0x00,0x09,0x04,0x10,0x00,0x00,0x10,0x70,0x17,0x00,0x00,0x11,0x88,0x13,0x00,0x00,0x12,0x78,0x1E,0x00,0x00,0x13,0x18,0xFC,0xFF,0xFF,0x14,0x00,0x00,0x00,0x00,0x00,0x00]
    crc = update_crc(data)
    data[len(data)-2] = crc & 0xff
    data[len(data)-1] = (crc >> 8) & 0xff
