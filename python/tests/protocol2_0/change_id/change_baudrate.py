...
groupSyncRead = GroupSyncRead(portHandler, packetHandler, 8, 1)
...
if SOURCE:
    ...
    for i in range(0,len(ID)):
        dxl_comm_result, dxl_error = packetHandler.write1ByteTxRx(portHandler, ID[i], 8, BAUD_RATE)
        if dxl_comm_result != COMM_SUCCESS:
            print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
        elif dxl_error != 0:
            print("%s" % packetHandler.getRxPacketError(dxl_error))
        else:
            print("Change PWM Limit:%d succeeded" %BAUD_RATE)
    ...
    for i in range(0, len(ID)):
        ...
        for i in range(0, len(ID)):
            ...
            baudrate = groupSyncRead.getData(ID[i], 8 ,1)
            baudrate = struct.unpack('i', struct.pack('I', baudrate))[0]

            print("[ID:%03d] baudrate:%03d" % (ID[i] ,baudrate))
