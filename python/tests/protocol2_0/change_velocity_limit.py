...
groupSyncRead = GroupSyncRead(portHandler, packetHandler, 44, 4)
...
if SOURCE:
    ...
    for i in range(0,len(ID)):
        dxl_comm_result, dxl_error = packetHandler.write4ByteTxRx(portHandler, ID[i], 44, VELOCITY_LIMIT)
        if dxl_comm_result != COMM_SUCCESS:
            print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
        elif dxl_error != 0:
            print("%s" % packetHandler.getRxPacketError(dxl_error))
        else:
            print("Change Velocity Limit:%d succeeded" %VELOCITY_LIMIT)
    ...
    for i in range(0, len(ID)):
        ...
        for i in range(0, len(ID)):
            velocitylimit = groupSyncRead.getData(ID[i], 44, 4)
            velocitylimit = struct.unpack('i', struct.pack('I', velocitylimit))[0]

            print("[ID:%03d] Velocity limit:%03d" % (ID[i] ,velocitylimit))
