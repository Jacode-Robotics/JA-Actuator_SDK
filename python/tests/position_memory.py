...
positionWrite = GroupSyncWrite(portHandler, packetHandler, ADDR_GOAL_POSITION, LEN_GOAL_POSITION)
    ...
    try:
        with open(filename, mode='a', newline='') as file:
            ...
            while flag:
                ...
                while label:
                    expect_time = int(input("Expected time:"))
                    if expect_time >= MINIMUM_TIME_LIMIT:
                        label = False
                    else:
                        print(f'Please enter a parameter greater than {MINIMUM_TIME_LIMIT}')
                label = True
                positions.append(expect_time)
...
# Set the motor status and write the position
def position_recurrence():
    for i in range(1,21):
        dxl_model_number,dxl_comm_result, dxl_error = packetHandler.ping(portHandler, i)
        if dxl_comm_result != COMM_SUCCESS:
            continue
    ...
    for i in range(0,data_length):
        ...
        for i in range(0, len(ID)): 
            ...
            dxl_addparam_result = positionWrite.addParam(ID[i], param_goal_position)
            result, error = packetHandler.write2ByteTxRx(portHandler, ID[i], ADDR_PROFILE_TIME, positions_data[-1])
            if dxl_addparam_result != True:
                print("[ID:%03d] groupSyncWrite addparam failed" % ID[i])
                quit()
        print(positions_data)
        dxl_comm_result = positionWrite.txPacket()
        if dxl_comm_result != COMM_SUCCESS:
            print("%s" % packetHandler.getTxRxResult(dxl_comm_result))

        # Clear syncwrite parameter storage
        positionWrite.clearParam()
...
# Record the location, and annotate it when reproducing it
    ''' 
